...
auto instruction() -> void {
  ...
  std::cout << "tri - if you want to check if it is possible to build a "
            ...
            << std::endl;
  std::cout << "fos1 - if u want field of square" << std::endl;
  std::cout << "fos2 - if u want field of square but with the use of a diagonal"
            << std::endl;
  ...
  std::cout << "fod - if u want field of deltoid" << std::endl;
  std::cout << "foc - if u want field of circle" << std::endl;
...
auto evaluate_addition() -> void {
  auto const b = ask_user_for_integer("Number 1: ");
  auto const a = ask_user_for_integer("Number 2: ");
...
auto evaluate_exponentiation() -> void {
  auto const b = ask_user_for_integer("Your number: ");
  auto const a = ask_user_for_integer("Exponent: ");
  std::cout << "Score: " << pow(b, a) << std::endl;
}

auto evaluate_square_root() -> void {
  auto const a = ask_user_for_integer("Your number: ");
  if (a < 0) {
...
auto evaluate_square_root_of_any_degree() -> void {
  auto const a = ask_user_for_integer("Your number: ");
  auto const b = ask_user_for_integer("The degree of the square: ");
  if (a < 0) {
    std::cerr << "There is no square root of a negative number. Try again!"
              << std::endl;
  }
...
auto evaluate_log() -> void {
  auto const a = ask_user_for_integer("Base of the logarithm: ");
  auto const b = ask_user_for_integer("Logarithmic number: ");
  if (a <= 0 || a == 1) {
...
auto evaluate_sine() -> void {
  auto const a = ask_user_for_integer("Your degrees: ");
  if (a < 0) {
...
auto evaluate_asine() -> void {
  double a = ask_user_for_integer("Your angle: ");
  if (a < -1.0 || a > 1.0) {
...
auto evaluate_acosine() -> void {
  auto const a = ask_user_for_integer("Your angle: ");
  if (a < -1.0 || a > 1.0) {
    std::cerr
...
auto evaluate_acotangent() -> void {
  auto const a = ask_user_for_integer("Your radians: ");
  if (a <= 0 || a >= M_PI) {
...
auto evaluate_compare() -> void {
  auto const a = ask_user_for_integer("Number 1: ");
  auto const b = ask_user_for_integer("Number 2: ");
  if (a > b) {
...
auto evaluate_is_it_triangle() -> void {
  auto const c = ask_user_for_integer("Side length a: ");
  auto const b = ask_user_for_integer("Side length b: ");
  auto const a = ask_user_for_integer("Side length c: ");
  if (a <= 0 || b <= 0 || c <= 0) {
...
auto evaluate_field_of_square1() -> void {
  auto const a = ask_user_for_integer("Side length a: ");
  if (a <= 0) {
    std::cerr << "The side lenght can't be negative" << std::endl;
  } else {
...
auto evaluate_field_of_rectangle() -> void {
  auto const b = ask_user_for_integer("Side length a: ");
  auto const a = ask_user_for_integer("Side length b: ");
  if (a <= 0 || b <= 0) {
    std::cerr << "The side lenght can't be negative" << std::endl;
  } else {
    std::cout << "Score: " << a * b << std::endl;
...
auto evaluate_field_of_triangle1() -> void {
  auto const h = ask_user_for_integer("Height: ");
  auto const a = ask_user_for_integer("Side length a: ");
  if (a <= 0 || h <= 0) {
...
auto evaluate_field_of_triangle2() -> void {
  auto const c = ask_user_for_integer("Side length c: ");
  auto const b = ask_user_for_integer("Side length b: ");
  auto const a = ask_user_for_integer("Side length a: ");
...
auto evaluate_field_of_rhombus() -> void {
  auto const f = ask_user_for_integer("Diagonal length f: ");
  auto const e = ask_user_for_integer("Diagonal lenght e: ");
  if (e <= 0 || f <= 0) {
...
auto evaluate_circuit_of_trapeze() -> void {
  auto const d = ask_user_for_integer("Side length d: ");
  auto const c = ask_user_for_integer("Side length c: ");
  auto const b = ask_user_for_integer("Side length b: ");
  auto const a = ask_user_for_integer("Side length a: ");
  if (a <= 0 || b <= 0 || c <= 0 || d <= 0) {
...
auto evaluate_field_of_deltoid() -> void {
  auto const d2 = ask_user_for_integer("Diagonal lenght d2: ");
  auto const d1 = ask_user_for_integer("Diagonal lenght d1: ");
  if (d1 <= 0 || d2 <= 0) {
    std::cerr << "The diagonal lenght can't be negative" << std::endl;
  } else {
...
auto evaluate_field_of_circle() -> void {
  auto const r = ask_user_for_integer("Ray: ");
  if (r <= 0) {
...
auto evaluate_field_of_cone() -> void {
  auto const l = ask_user_for_integer("Generatrix: ");
  auto const r = ask_user_for_integer("Ray: ");
  if (r <= 0 || l <= 0) {
...
auto main(int argc, char *argv[]) -> int {
  ...
  std::string each = ask_user_for_credential("What do you wanna do: ");
